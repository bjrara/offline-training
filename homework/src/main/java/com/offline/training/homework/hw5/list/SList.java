...
public class SList<T> {
  ...
  public void insertEnd(T obj) {
    if (head == null) {
      head = new SListNode<T>(obj);
      tail = head;
    } else {
      tail.next = new SListNode<T>(obj);
      tail = tail.next;
    }
    size++;
  }
   ...
   *  @param position the desired position, from 0 to length() - 1, in the list.
  ...
  public T nth(int position) {
    ...
    if (position < 0 || head == null || position > size - 1) {
      return null;
    } else {
      currentNode = head;
      while (position > 0) {
  ...
  public void remove(int position) {
	SListNode<T> currentNode, previousNode;
	if ((position < 0) || (head == null) || position > size - 1) {
		return;
	} else {
		if (position == 0) {
  ...
  private static void testEmpty() {
    SList<Integer> lst1 = new SList<Integer>();
    SList<Integer> lst2 = new SList<Integer>();
    System.out.println();
