package com.offline.training.homework.hw5;
/* SimpleBoard.java */
...
public class SimpleBoard {
  ...
  public boolean equals(Object board) {
	  ...
	  if (!(board instanceof SimpleBoard))
		  return false;
	  SimpleBoard another = (SimpleBoard) board;
	  for (int i = 0; i < DIMENSION; i++) {
		  for (int j = 0; j < DIMENSION; j++) {
			  if (elementAt(i, j) != another.elementAt(i, j))
				  return false;
		  }
	  }
	  return true;
  ...
  public int hashCode() {
	  ...
	  for (int i = 0; i < DIMENSION; i++) {
		  for (int j = 0; j < DIMENSION; j++) {
			  base3Value += Math.pow(3, pos) * elementAt(i, j);
			  pos++;
		  }
	  }
	  return (int)(base3Value & 0x7FFFFFFF);
  }
